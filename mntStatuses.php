<?php
//
//  mntStatuses.php is a PHP component of HPTTix
...
// CHANGE TO FIT PARTICULAR TABLE. Post edited item to database
	if ($DEFAULT == 1) {
		$sql2 = "UPDATE defaults SET defStatus = $ID where id = 1";
		$result2 = mysql_query($sql2);
		$ACTIVE = 1;
	}
	$sql = "UPDATE statuses SET name = '$NAME', active = $ACTIVE ";
	$sql .= "WHERE id = $ID";
...
function editThingForm() {
	...
	include('dbconn.php');
// THE NAMES PAST HERE MUST BE CHANGED TO FIT THE PARTICULAR ROUTINE.  DON'T FORGET FUNC NAMES, ETC.
	$query = "SELECT defStatus from defaults where id = 1";
	$result = mysql_query($query);
	if ($result) {
		$row = mysql_fetch_array($result);
		$defaultValue = $row["defStatus"];
	}
	$query = "SELECT ID, NAME, ACTIVE ";
	$query .= " from statuses where id = $id";
	$result = mysql_query($query);
	if ($result) {
		$row = mysql_fetch_array($result);	
		$id = $row["ID"];
		$name = $row["NAME"];
		$active = $row["ACTIVE"];
		print "<form action=\"index.php?func=mntStatuses&page=$pageNum\" method=\"post\">\n";
...
function displayThings() {
	...
	print "<tr><th>Status</th><th>Default?</th><th>Active?</th></tr>\n";
	$query = "SELECT ID, NAME, ACTIVE from statuses order by name LIMIT $offset, $rowsPerPage";
	$result = mysql_query($query);
	if ($result) {
		while ($row = mysql_fetch_array($result)) {
			...
			print "<td><a href=\"index.php?func=mntStatuses&editThing=$id&page=$pageNum\">edit</a></td>";
			print "</tr>\n";
		}
	}
	print "</table><br />\n";
	...
	if ($pageNum > 1) {
		$page = $pageNum - 1;
		$prev = " <a href=\"$self?func=mntStatuses&page=$page\">[Prev Page]</a> ";
		$first = " <a href=\"$self?func=mntStatuses&page=1\">[First Page]</a> ";
	} else {
	...
	if ($pageNum < $maxPage) {
		$page = $pageNum + 1;
		$next = " <a href=\"$self?func=mntStatuses&page=$page\">[Next Page]</a> ";
		$last = " <a href=\"$self?func=mntStatuses&page=$maxPage\">[Last Page]</a> ";
	} else {
...
function validInsert() {
    ...
    $query = "SELECT id FROM statuses WHERE name = '$NAME'";
    $result = mysql_query($query);
...
function displayThingForm() {
	global $thing, $things;
	print "<form action=\"index.php?func=mntStatuses\" method=\"post\">\n";
	print "<table width=\"80%\" align=\"center\">";
	print "<tr><td align=\"right\">$thing: </td>";
