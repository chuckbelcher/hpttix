<?php
//
//  mntSeverities.php is a PHP component of HPTTix
...
// CHANGE TO FIT PARTICULAR TABLE. Post edited item to database
	if ($DEFAULT == 1) {
		$sql2 = "UPDATE defaults SET defSeverity = $ID where id = 1";
		$result2 = mysql_query($sql2);
		$ACTIVE = 1;
	}
	$sql = "UPDATE severities SET name = '$NAME', active = $ACTIVE ";
	$sql .= "WHERE id = $ID";
...
function editThingForm() {
	...
	include('dbconn.php');
// THE NAMES PAST HERE MUST BE CHANGED TO FIT THE PARTICULAR ROUTINE.  DON'T FORGET FUNC NAMES, ETC.
	$query = "SELECT defSeverity from defaults where id = 1";
	$result = mysql_query($query);
	if ($result) {
		$row = mysql_fetch_array($result);
		$defaultValue = $row["defSeverity"];
	}
	$query = "SELECT ID, NAME, ACTIVE ";
	$query .= " from severities where id = $id";
	$result = mysql_query($query);
	if ($result) {
		$row = mysql_fetch_array($result);	
		$id = $row["ID"];
		$name = $row["NAME"];
		$active = $row["ACTIVE"];
		print "<form action=\"index.php?func=mntSeverities&page=$pageNum\" method=\"post\">\n";
...
function displayThings() {
	...
	$query = "SELECT ID, NAME, ACTIVE from severities order by name LIMIT $offset, $rowsPerPage";
	$result = mysql_query($query);
	if ($result) {
		while ($row = mysql_fetch_array($result)) {
			...
			print "<td><a href=\"index.php?func=mntSeverities&editThing=$id&page=$pageNum\">edit</a></td>";
			print "</tr>\n";
		}
	}
	print "</table><br />\n";
	// how many rows we have in database
	$query = "SELECT COUNT(*) AS numrows FROM severities";
	...
	if ($pageNum > 1) {
		$page = $pageNum - 1;
		$prev = " <a href=\"$self?func=mntSeverities&page=$page\">[Prev Page]</a> ";
		$first = " <a href=\"$self?func=mntSeverities&page=1\">[First Page]</a> ";
	} else {
	...
	if ($pageNum < $maxPage) {
		$page = $pageNum + 1;
		$next = " <a href=\"$self?func=mntSeverities&page=$page\">[Next Page]</a> ";
		$last = " <a href=\"$self?func=mntSeverities&page=$maxPage\">[Last Page]</a> ";
	} else {
...
function validInsert() {
    ...
    $query = "SELECT id FROM severities WHERE name = '$NAME'";
    $result = mysql_query($query);
...
function displayThingForm() {
	global $thing, $things;
	print "<form action=\"index.php?func=mntSeverities\" method=\"post\">\n";
	print "<table width=\"80%\" align=\"center\">";
	print "<tr><td align=\"right\">$thing: </td>";
